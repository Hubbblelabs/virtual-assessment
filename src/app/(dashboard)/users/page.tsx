...
export default function UsersPage() {
  ...
  const fetchGroups = async () => {
    try {
      ...
      // Ensure we don't leave it in a bad state if it was somehow initialized poorly
      setGroups([]);
    }
  };
  ...
  const getAssignedGroups = (teacherId: string) => {
    ...
    return groups.filter(group =>
      group.teachers && group.teachers.some(teacher => teacher._id === teacherId)
    );
  };
                          ...
                          return (
                            <div className="text-sm">
                              {assignedGroups.length > 0 ? (
                                ...
                                <span className="text-muted-foreground italic">No groups assigned</span>
                              )}
                            </div>
                          );
                        })()
